import{n as e}from"./vendor-BuoCFfzO.js";import{d as t,l as n,o as r,r as i}from"./widev-DfeMkeQH.js";import{Ht as a,L as o,T as s,V as c,et as l,qt as u}from"./firebase-WP_1NxOL.js";import{n as d}from"./firebase-jIxv3xsb.js";var f=()=>t(`wiSmile`),p=[],m=`todos`,h=()=>{let e=f();return!e||e.rol!==`admin`?`<div class="adu_page"><div class="adu_empty"><i class="fas fa-ban"></i><p>Acceso denegado.</p></div></div>`:`
  <div class="adu_page">
    ...
    <div id="adu_modales"></div>
  ...
  </div>`},g=async()=>{let t=f();!t||t.rol!==`admin`||(e(document).off(`.adu`),await v(),e(document).on(`input.adu`,`#adu_input_search`,function(){y()}),e(document).on(`click.adu`,`.adu_filter_btn`,function(){e(`.adu_filter_btn`).removeClass(`active`),e(this).addClass(`active`),m=e(this).data(`rol`),y()}),e(document).on(`click.adu`,`#adu_btn_sync`,async function(){let t=e(this).find(`i`).addClass(`fa-spin`);await v(!0),setTimeout(()=>t.removeClass(`fa-spin`),500)}),e(document).on(`click.adu`,`.adu_btn_ico.danger`,async function(){let t=e(this).data(`id`);if(confirm(`¿Atención! Eliminarás a "${t}" de forma permanente. ¿Continuar?`))try{await s(u(d,`smiles`,t)),i(`Usuario eliminado`,`info`),await v(!0)}catch{i(`Error al eliminar`,`error`)}}),e(document).on(`click.adu`,`.adu_btn_ico.view`,function(){let t=e(this).data(`id`),n=p.find(e=>e.id===t);n&&b(n)}),e(document).on(`click.adu`,`.adu_modal_close, .adu_btn_cancel`,()=>e(`#adu_modales`).html(``)),e(document).on(`click.adu`,`.adu_modal_bg`,t=>{e(t.target).hasClass(`adu_modal_bg`)&&e(`#adu_modales`).html(``)}))},_=()=>{e(document).off(`.adu`)};async function v(t=!1){try{p=(await o(l(a(d,`smiles`),c(300)))).docs.map(e=>({id:e.id,usuario:e.id,...e.data()})),y()}catch(t){console.error(`[admin_usuarios] Error:`,t),e(`#adu_table_body`).html(`<tr><td colspan="5"><div class="adu_empty" style="padding:4vh"><i class="fas fa-exclamation-triangle"></i><p>Error de conexión.</p></div></td></tr>`)}}function y(){let t=(e(`#adu_input_search`).val()||``).toLowerCase(),i=p.filter(e=>!(m!==`todos`&&(e.rol||`smile`)!==m||t&&![e.id,e.email,e.nombre,e.nombres,e.apellidos].join(` `).toLowerCase().includes(t)));if(!i.length){e(`#adu_table_body`).html(`<tr><td colspan="5"><div class="adu_empty" style="padding:4vh"><i class="fas fa-user-slash"></i><p>No se encontraron usuarios.</p></div></td></tr>`);return}let a=i.map(e=>{let t=e.nombres||e.nombre||e.id,i=e.email||`—`,a=r(t),o=e.rol||`smile`,s=e.fecha?.toDate?n(e.fecha):`—`;return`
      <tr class="adu_row">
        <td>
      ...
      </tr>`}).join(``);e(`#adu_table_body`).html(a)}function b(t){let r=t.fecha?.toDate?n(t.fecha):`Desconocido`;e(`#adu_modales`).html(`

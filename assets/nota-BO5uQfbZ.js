...
import{n as e}from"./vendor-BuoCFfzO.js";import{i as t,n}from"./rutas-DP-LLXJj.js";var r=`zenwii`,i=`Zenwii`;(()=>{let t=0;return(n=`[data-herowi]`,r=45)=>{t||=(e(`head`).append(`<style>
        @keyframes hwi_fade {
...
`;function w(e=``){if(!e)return``;let t=e;return t=t.replace(/\r/g,``),t=t.replace(/<h1[^>]*>(.*?)<\/h1>/gi,`# $1
...
`,j=async()=>{let{db:e}=await t(async()=>{let{db:e}=await import(`./firebase-CiMLHh51.js`);return{db:e}},[]);return{db:e,...await t(()=>import(`./firebase-D3TdL7f_.js`).then(e=>e.d),[])}},M=(e,t)=>t?.usuario?t.usuario.toLowerCase().replace(/[^a-z0-9]/g,``)+(e.id.startsWith(`wd`)?e.id.replace(`wd`,``):e.id):e.id,N=async e=>{let t=s.user;if(t?.email)try{let{db:n,doc:r,setDoc:i,serverTimestamp:a}=await j(),o=e.creado?new Date(e.creado):a(),s=e.actualizado?new Date(e.actualizado):a();await i(r(n,`notas`,M(e,t)),{id:e.id,usuario:t.usuario||t.email.split(`@`)[0],email:t.email,titulo:String(e.titulo||``),contenido:String(e.contenido||``),contenidoMd:w(e.contenido||``),pin:!!e.pin,creado:o,actualizado:s})}catch(e){console.error(`[notas] guardarNube:`,e)}},P=async e=>{let t=s.user;if(t?.email)try{let{db:n,doc:r,setDoc:i,serverTimestamp:a}=await j(),o=e.actualizado?new Date(e.actualizado):a();await i(r(n,`notas`,M(e,t)),{id:e.id,usuario:t.usuario||t.email.split(`@`)[0],email:t.email,titulo:String(e.titulo||``),contenido:String(e.contenido||``),contenidoMd:w(e.contenido||``),pin:!!e.pin,actualizado:o},{merge:!0})}catch(e){console.error(`[notas] actualizarNube:`,e)}},F=async e=>{let t=s.user;if(t?.email)try{let{db:n,doc:r,deleteDoc:i}=await j();await i(r(n,`notas`,M({id:e},t)))}catch(e){console.error(`[notas] eliminarNube:`,e)}},I=async()=>{let e=s.user;if(!e?.email)return null;try{let{db:t,collection:n,getDocs:r,query:i,where:a}=await j();return(await r(i(n(t,`notas`),a(`email`,`==`,e.email)))).docs.map(e=>{let t=e.data();return{id:t.id||e.id,titulo:t.titulo||``,contenido:t.contenido||(t.contenidoMd?T(t.contenidoMd):``),contenidoMd:t.contenidoMd||``,pin:!!t.pin,creado:t.creado?.toMillis?.()||t.creado||Date.now(),actualizado:t.actualizado?.toMillis?.()||t.actualizado||Date.now(),synced:!0}})}catch(e){return console.error(`[notas] cargarNube:`,e),null}},L=(e,t)=>{let n=b(e.contenido),r=n.length>50?n.substring(0,50)+`...`:n||`Sin contenido...`,i=e.id===t?`active`:``,a=e.titulo||`Documento sin título`;return`
    <div class="wd_doc_item ${i}${e.pin?` wd_pinned`:``}" data-id="${e.id}">
      ...
      <p>${r}</p>
    </div>`},R=null,z=null;n.navigate=e=>{console.log(`[Zenwii Editor] Ruta interceptada:`,e),e!==`/nota`&&(window.location.href=e)},e(`#winota`).html(A()),(async()=>{let n=k.get(),r=null,o=null,c=[],f=e=>{if(n=n.filter(t=>t.id!==e),c=c.filter(t=>t!==e),localStorage.setItem(`open_tabs`,JSON.stringify(c)),k.set(n),s.user&&F(e),u(`Nota eliminada permanentemente`,`success`),r?.id===e)if(c.length>0){let e=n.find(e=>e.id===c[0]);e&&M(e)}else n.length>0?M(O()[0]):B();else A()};try{let e=localStorage.getItem(`open_tabs`);e&&(c=JSON.parse(e))}catch{}let p=()=>{let t=e(`#wd_tabs_bar`);if(!t.length)return;if(c=c.filter(e=>n.some(t=>t.id===e)),c.length===0&&r&&c.push(r.id),localStorage.setItem(`open_tabs`,JSON.stringify(c)),c.length===0){t.html(``);return}let i=`<button id="wd_btn_tab_search" class="wd_tab_search_btn" ${m(`Buscar nota por título`,void 0,`bottom`)}><i class="fas fa-magnifying-glass"></i></button>`+c.map(e=>{let t=n.find(t=>t.id===e);if(!t)return``;let i=t.id===r?.id?`active`:``,a=t.titulo||`Sin título`;return`
        ...
        </div>`}).join(``)+`<div class="wd_tab_sep_new"></div><button id="wd_btn_new_tab" class="wd_tab_new_btn" ${m(`Nuevo Documento`,void 0,`bottom`)}><i class="fas fa-plus"></i></button>`;t.html(i)},x=t=>{if(!s.user||!t){e(`#wd_meta, .wd_meta_sep_main`).hide();return}e(`#wd_meta, .wd_meta_sep_main`).css(`display`,`flex`),e(`#wd_meta_rel span`).text(y(t.actualizado||t.creado)),e(`#wd_meta_cre span`).text(v(t.creado)),e(`#wd_meta_upd span`).text(v(t.actualizado))},C=t=>{clearInterval(R),!(!s.user||!t)&&(R=setInterval(()=>{e(`#wd_meta_rel span`).text(y(t.actualizado||t.creado))},6e4))},T=()=>{e(`#wd_sb_list`).html(`
      <div class="wd_skeleton_item">
        <div class="wd_skeleton wd_sk_title"></div>
        <div class="wd_skeleton wd_sk_body"></div>
      </div>
    `.repeat(3))},O=()=>[...n].sort((e,t)=>e.pin&&!t.pin?-1:!e.pin&&t.pin?1:(t.actualizado||0)-(e.actualizado||0)),A=async()=>{let e=O();await _(`#wd_sb_list`,e.length?e.map(e=>L(e,r?.id)).join(``):`<div class="wd_empty">No tienes documentos. Crea uno nuevo.</div>`,80),p()},M=t=>{r=t,e(`#wd_in_tit`).val(t.titulo||``),e(`#wd_editor`).html(t.contenido||``),c.includes(t.id)||c.push(t.id),localStorage.setItem(`open_tabs`,JSON.stringify(c)),A(),x(t),C(t),S(s.user,r)},B=()=>{n=n.filter(e=>!(e.id.startsWith(`ej`)&&(e.titulo===`Documento de Ejemplo`||e.titulo===``)));let t={id:D(),titulo:``,contenido:``,contenidoMd:``,pin:!1,creado:Date.now(),actualizado:Date.now(),synced:!1};n.unshift(t),k.set(n),M(t),setTimeout(()=>{e(`#wd_in_tit`).val(``).focus(),document.execCommand(`fontName`,!1,`'Segoe UI', system-ui`)},50)},V=(t=``)=>{let i=t.trim().toLowerCase(),a=[];a=i?n.filter(e=>{let t=(e.titulo||``).toLowerCase().includes(i),n=b(e.contenido||``).toLowerCase().includes(i);return t||n}):O().slice(0,5);let o=e(`#wd_search_results`);if(a.length===0){o.html(`<div class="wd_search_no_results">No se encontraron notas que coincidan con la búsqueda.</div>`);return}let s=a.map(e=>{let t=r&&r.id===e.id?`active`:``,n=e.pin?`<i class="fas fa-thumbtack wd_search_result_icon pinned"></i>`:`<i class="fa-solid fa-file-lines wd_search_result_icon"></i>`,i=e.titulo||`Sin título`,a=b(e.contenido),o=a.length>80?a.substring(0,80)+`...`:a||`Sin contenido...`,s=y(e.actualizado||e.creado);return`

import{n as e}from"./vendor-BuoCFfzO.js";import{t}from"./wii-DQBqtwtl.js";import{f as n,h as r,n as i,p as a,s as o,y as s}from"./rutas-Cm_3kJJL.js";import{a as c,c as l,f as u,p as d}from"./firebase-D_Qc_hgv.js";import{n as f}from"./firebase-CkLzmU2G.js";var p=()=>{let e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}`},m=async e=>{try{let t=`empleadosPuntos_${e}`,n=a(t);if(n)return console.log(`🏆 Ranking del mes ${e} recuperado de caché local.`),n;console.log(`🔄 Calculando ranking en vivo desde Firestore para el mes ${e}...`);let i=(await c(l(d(f,`smiles`),u(`participa`,`==`,`si`)))).docs.map(e=>{let t=e.data();return{usuario:t.usuario||e.id,nombre:t.nombre||t.usuario||e.id,descripcion:t.descripcion||`Colaborador`,imagen:t.imagen||``,totalPuntos:0,totalVentas:0}}),o=await c(d(f,`registrosdb`)),[s,p]=e.split(`-`).map(Number);return o.docs.forEach(e=>{let t=e.data(),n=t.fechaTour,r,a;if(typeof n==`string`)[r,a]=n.split(`-`).map(Number);else if(n?.toDate){let e=n.toDate();r=e.getFullYear(),a=e.getMonth()+1}else return;if(r===s&&a===p){let e=i.find(e=>e.usuario===t.vendedor);e&&(e.totalPuntos+=parseInt(t.puntos)||0,e.totalVentas+=parseInt(t.qventa)||1)}}),i.sort((e,t)=>t.totalPuntos===e.totalPuntos?t.totalVentas===e.totalVentas?e.nombre.localeCompare(t.nombre):t.totalVentas-e.totalVentas:t.totalPuntos-e.totalPuntos),r(t,i,5),i}catch(e){return console.error(`Error en obtenerRankingMes (zsmile):`,e),[]}},h=p(),g=()=>`
    <div class="smw_dash">
  ...
  `,_=async()=>{let r=s.user;if(!r)return setTimeout(()=>i.navigate(`/login`),100);let a=n(r.nombre||r.usuario||``),c=`${(r.nombre||`?`)[0]}${(r.apellidos||``)[0]||``}`.toUpperCase();e(`#smwAvatar`).text(c),e(`#smwSaludo`).html(`${o()} <strong>${a}</strong>`),r.descripcion?e(`#smwRole`).html(`<i class="fas fa-user-tag"></i> ${r.descripcion}`):e(`#smwRole`).html(`<i class="fas fa-car-side"></i> Colaborador — Reto del Mes`),e(`#smwMonthSelector`).val(h),y(r.usuario,h),e(document).off(`change.smile_dash`).on(`change.smile_dash`,`#smwMonthSelector`,function(){h=e(this).val(),y(r.usuario,h)}),e(`.wi_fadeUp`).addClass(`visible wi_visible`),console.log(`🏜️ ${t} Smile Dashboard cargado`),window.__WIREADY__=!0},v=()=>{e(document).off(`change.smile_dash`)};async function y(t,n){try{e(`#kpiTours`).html(`<span class="smw_sk_kpi"></span>`),e(`#kpiPuntos`).html(`<span class="smw_sk_kpi"></span>`),e(`#kpiPos`).html(`<span class="smw_sk_kpi"></span>`);let i=`kpiSmile_${t}_${n}`,o=a(i);if(o)return b(o);let[s,l]=n.split(`-`).map(Number),u=await c(d(f,`registrosdb`)),p=0,h=0;u.docs.forEach(e=>{let n=e.data();if(n.vendedor!==t)return;let r=n.fechaTour,i,a;if(typeof r==`string`)[i,a]=r.split(`-`).map(Number);else if(r?.toDate){let e=r.toDate();i=e.getFullYear(),a=e.getMonth()+1}else return;i===s&&a===l&&(p+=parseInt(n.qventa)||1,h+=parseInt(n.puntos)||0)});let g=(await m(n)).findIndex(e=>e.usuario===t),_=g===-1?`—`:`#${g+1}`,v={tours:p,puntos:h,posicion:_};r(i,v,5),b(v)}catch(e){console.warn(`KPI error:`,e),b({tours:`?`,puntos:`?`,posicion:`?`})}}function b({tours:t,puntos:n,posicion:r}){e(`#kpiTours`).text(t),e(`#kpiPuntos`).text(n),e(`#kpiPos`).text(r)}function x(){let t=new Date,n=t.getFullYear(),r=t.getMonth(),i=[`Enero`,`Febrero`,`Marzo`,`Abril`,`Mayo`,`Junio`,`Julio`,`Agosto`,`Septiembre`,`Octubre`,`Noviembre`,`Diciembre`];return e.map(Array(7),(e,t)=>{let a=t-3,o=r+a,s=n+Math.floor(o/12),c=(o%12+12)%12;return`<option value="${`${s}-${String(c+1).padStart(2,`0`)}`}"${a===0?` selected`:``}>${i[c]} ${s}</option>`}).join(``)}export{v as cleanup,_ as init,g as render};
...
`,C=null,w=null,T=60;function E(){w&&clearInterval(w);let e=localStorage.getItem(`vault_expire`);e||(e=Date.now()+T*1e3,localStorage.setItem(`vault_expire`,e));let i=()=>Math.max(0,Math.ceil((parseInt(e)-Date.now())/1e3)),a=i();t(`#vault_timer`).text(`${a}s`);let o=async()=>{if(a=i(),t(`#vault_timer`).text(`${a}s`),a<=0){w&&clearInterval(w),w=null,localStorage.removeItem(`vault_expire`),n(`Sesión cerrada por inactividad`,`error`);let{salir:e}=await r(async()=>{let{salir:e}=await import(`./login-BE6mkXdb.js`);return{salir:e}},__vite__mapDeps([0]));await e()}};o(),a>0&&(w=setInterval(o,1e3))}var D=()=>{let e=a(`wiSmile`);return!e||e.rol!==`admin`?``:`

...
</div></main>`,y=()=>{e(document).on(`input.contacto`,`#ct_mensaje`,function(){let t=e(this).val();t.length>f&&e(this).val(t.slice(0,f)),e(`#ct_count`).text(Math.min(t.length,f))}),e(document).on(`reset.contacto`,`#ctForm`,()=>{setTimeout(()=>e(`#ct_count`).text(`0`),10)}),e(document).on(`submit.contacto`,`#ctForm`,async function(n){if(n.preventDefault(),e(`#ct_honey`).val())return;if(!h()){r(`Espera un momento antes de enviar otro mensaje.`,`warning`);return}let i=e(`#ct_nombre`).val().trim(),a=e(`#ct_email`).val().trim(),s=e(`#ct_asunto`).val(),l=e(`#ct_mensaje`).val().trim();if(i.length<3)return r(`El nombre debe tener al menos 3 caracteres.`,`error`);if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a))return r(`Ingresa un email válido.`,`error`);if(!s)return r(`Selecciona una incidencia.`,`error`);if(l.length<10)return r(`El mensaje debe tener al menos 10 caracteres.`,`error`);let u=e(`#ct_submit`);o(u,!0,`Enviando…`);try{window.emailjs===void 0&&await new Promise((e,t)=>{let n=document.createElement(`script`);n.src=`https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js`,n.onload=e,n.onerror=()=>t(Error(`No se pudo cargar EmailJS`)),document.head.appendChild(n)}),window.emailjs.init(c.pub),await window.emailjs.send(c.sid,c.tid,{nombre:i,email:a,telefono:`N/A`,asunto:s,mensaje:l,app_name:t}),g(),r(`¡Mensaje enviado al equipo de CumpleWii! Te responderemos pronto. 💌`,`success`,4500),this.reset(),e(`#ct_count`).text(`0`)}catch(e){console.error(`[contacto] EmailJS error:`,e),r(`No se pudo enviar el mensaje. Revisa tu conexión o intenta más tarde.`,`error`)}finally{o(u,!1,`Enviar Mensaje`)}}),e(document).on(`click.contacto`,`.ct_copy`,function(){n(e(this).data(`copy`),this,`¡Copiado!`)}),e(document).on(`click.contacto`,`.ct_faq_q`,function(){let t=e(this).closest(`.ct_faq_item`),n=t.hasClass(`active`);e(`.ct_faq_item`).removeClass(`active`).find(`.ct_faq_a`).slideUp(280),e(`.ct_faq_arr`).removeClass(`rotated`),n||(t.addClass(`active`).find(`.ct_faq_a`).slideDown(280),t.find(`.ct_faq_arr`).addClass(`rotated`))}),_.push(a(`.wi_fadeUp`,t=>e(t).addClass(`visible`))),_.push(a(`.ct_faq_item`,(t,n)=>setTimeout(()=>e(t).addClass(`visible`),n*80))),console.log(`📩 ${t} Contacto cargado`),window.__WIREADY__=!0},b=()=>{e(document).off(`.contacto`),_.forEach(e=>e?.disconnect?.()),_=[]};export{b as cleanup,y as init,v as render};
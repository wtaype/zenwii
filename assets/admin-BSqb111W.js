import{n as e}from"./vendor-BuoCFfzO.js";import{t}from"./wii-DQBqtwtl.js";import{d as n,l as r,n as i}from"./widev-BX5_JJ6V.js";import{d as a,f as o,n as s,s as c}from"./firebase-BuEyh8Mq.js";import{n as l}from"./firebase-CKBWl3dx.js";var u=()=>r(`wiSmile`),d=`amUsersTotal`,f=`amUsersPro`,p=`amOrgsTotal`,m=`amLessonsTotal`,h=()=>{let e=u();return!e||e.rol!==`admin`?`<div class="am_page"><div class="am_empty"><i class="fas fa-ban"></i> Acceso denegado.</div></div>`:`
  <div class="am_page">
  ...
  </div>`},g=async()=>{let t=u();!t||t.rol!==`admin`||(e(document).off(`.am`),v(),y(),e(document).on(`click.am`,`#am_btn_refresh`,function(){let t=e(this).find(`i`).addClass(`fa-spin`);y(!0).then(()=>{setTimeout(()=>t.removeClass(`fa-spin`),500)})}))},_=()=>{e(document).off(`.am`)};function v(){e(`#am_n_total`).text(r(d)||`—`),e(`#am_n_pro`).text(r(f)||`—`),e(`#am_n_org`).text(r(p)||`—`),e(`#am_n_les`).text(r(m)||`45`)}async function y(t=!1){try{let u=o(l,`smiles`);if(t||!r(d)){let t=(await s(u)).data().count;n(d,t),e(`#am_n_total`).text(t)}if(t||!r(f)){let t=(await s(c(u,a(`rol`,`in`,[`gestor`,`empresa`,`admin`])))).data().count;n(f,t),e(`#am_n_pro`).text(t)}if(t||!r(p)){let t=(await s(c(u,a(`rol`,`==`,`empresa`)))).data().count;n(p,t),e(`#am_n_org`).text(t)}t&&i(`Estadísticas sincronizadas`,`success`)}catch(e){console.error(`[Admin] Error cargando stats:`,e),t&&i(`Error sincronizando`,`error`)}}export{_ as cleanup,g as init,h as render};
...
`,C=(t,n)=>{let r=e(`#wd_hdr_auth`);if(r.length)if(t){let e=localStorage.getItem(`wd_realtime_sync`)!==`off`;n?.synced;let i=e?`Sincronización en tiempo real: ON — haz clic para pausar`:`Sincronización en tiempo real: OFF — haz clic para activar`,a=t.avatar||`/zenwii/smile.avif`;r.html(`
      <div class="wd_hdr_user_info" style="display:flex; gap:1.5vh; align-items:center;">
        <label class="wd_rt_toggle" ${m(i,void 0,`bottom`)} title="${i}">
          <input type="checkbox" id="wd_rt_sync_toggle" ${e?`checked`:``} />
          <span class="wd_rt_track">
            <i class="fas ${e?`fa-cloud`:`fa-cloud-arrow-up`} wd_rt_icon"></i>
          </span>
        </label>
        <a href="/perfil" target="_blank" class="nv_item" ${m(`Ver mi perfil`,void 0,`bottom`)}>
          <img src="${a}" alt="${t.nombre||t.usuario}">
    ...
    `);let o=document.getElementById(`wd_rt_sync_toggle`);o&&o.addEventListener(`change`,()=>{let e=o.checked;localStorage.setItem(`wd_realtime_sync`,e?`on`:`off`);let t=o.closest(`.wd_rt_toggle`)?.querySelector(`.wd_rt_icon`);t&&(t.className=`fas ${e?`fa-cloud`:`fa-cloud-arrow-up`} wd_rt_icon`);let n=e?`Sincronización en tiempo real: ON — haz clic para pausar`:`Sincronización en tiempo real: OFF — haz clic para activar`;o.closest(`.wd_rt_toggle`)?.setAttribute(`title`,n),window.dispatchEvent(new CustomEvent(`wd_rt_sync_changed`,{detail:{on:e}}))})}else r.html(`
    ...
    `)},w=()=>!window.isRel&&(window.isRel=1)&&t(()=>import(`./login-mTiX_rkD.js`).then(e=>e.salir([`wiTema`,`wiSmart`,`cookiesPrivacidad`,`superFun`]).then(()=>location.reload())),__vite__mapDeps([0]));x(async()=>{let[{auth:e},{onAuthStateChanged:n}]=await Promise.all([t(()=>import(`./firebase-BBmvSH75.js`),[]),t(()=>import(`./firebase-D_Qc_hgv.js`).then(e=>e.v),[])]);n(e,e=>!e&&s.user&&w())}),window.addEventListener(`storage`,e=>(!e.key||e.key===`wiSmile`)&&location.reload());var T=()=>`
...
`,N=async()=>{let{db:e}=await t(async()=>{let{db:e}=await import(`./firebase-BBmvSH75.js`);return{db:e}},[]);return{db:e,...await t(()=>import(`./firebase-D_Qc_hgv.js`).then(e=>e.t),[])}},P=(e,t)=>{if(!t?.usuario)return e.id;let n=t.usuario||t.email||`nota`;return n.includes(`@`)&&(n=n.split(`@`)[0]),`${n.toLowerCase().replace(/[^a-z0-9_-]/g,``)}_${e.creado||Date.now()}`},F=async e=>{let n=s.user;if(!n?.email)return e.id;try{let{db:r,doc:i,setDoc:a,serverTimestamp:o}=await N(),s=e.creado?new Date(e.creado):o(),c=e.actualizado?new Date(e.actualizado):o(),l=P(e,n);if(e.id&&e.id!==l)try{let{deleteDoc:n}=await t(async()=>{let{deleteDoc:e}=await import(`./firebase-D_Qc_hgv.js`).then(e=>e.t);return{deleteDoc:e}},[]);await n(i(r,`notas`,e.id))}catch(e){console.warn(`[delete-old-remote-failed]`,e)}return await a(i(r,`notas`,l),{id:l,usuario:n.usuario?n.usuario.toLowerCase().replace(/[^a-z0-9_-]/g,``):n.email.split(`@`)[0],email:n.email,titulo:String(e.titulo||``),contenido:String(e.contenido||``),contenidoMd:E(e.contenido||``),pin:!!e.pin,creado:s,actualizado:c}),l}catch(t){return console.error(`[notas] guardarNube:`,t),e.id}},I=async e=>{let n=s.user;if(!n?.email)return e.id;try{let{db:r,doc:i,setDoc:a,serverTimestamp:o}=await N(),s=e.actualizado?new Date(e.actualizado):o(),c=P(e,n);if(e.id&&e.id!==c)try{let{deleteDoc:n}=await t(async()=>{let{deleteDoc:e}=await import(`./firebase-D_Qc_hgv.js`).then(e=>e.t);return{deleteDoc:e}},[]);await n(i(r,`notas`,e.id))}catch(e){console.warn(`[delete-old-remote-failed]`,e)}return await a(i(r,`notas`,c),{id:c,usuario:n.usuario?n.usuario.toLowerCase().replace(/[^a-z0-9_-]/g,``):n.email.split(`@`)[0],email:n.email,titulo:String(e.titulo||``),contenido:String(e.contenido||``),contenidoMd:E(e.contenido||``),pin:!!e.pin,actualizado:s},{merge:!0}),c}catch(t){return console.error(`[notas] actualizarNube:`,t),e.id}},L=async e=>{let t=s.user;if(t?.email)try{let{db:n,doc:r,deleteDoc:i}=await N(),a=P({id:e},t);if(await i(r(n,`notas`,a)),a.includes(`_`)){let e=a.replace(`_`,``);try{await i(r(n,`notas`,e))}catch{}}}catch(e){console.error(`[notas] eliminarNube:`,e)}},R=async()=>{let e=s.user;if(!e?.email)return null;try{let{db:t,collection:n,getDocs:r,query:i,where:a}=await N();return(await r(i(n(t,`notas`),a(`email`,`==`,e.email)))).docs.map(e=>{let t=e.data();return{id:t.id||e.id,titulo:t.titulo||``,contenido:t.contenido||(t.contenidoMd?D(t.contenidoMd):``),contenidoMd:t.contenidoMd||``,pin:!!t.pin,creado:t.creado?.toMillis?.()||t.creado||Date.now(),actualizado:t.actualizado?.toMillis?.()||t.actualizado||Date.now(),synced:!0}})}catch(e){return console.error(`[notas] cargarNube:`,e),null}},z=(e,t)=>{let n=b(e.contenido),r=n.length>50?n.substring(0,50)+`...`:n||`Sin contenido...`,i=e.id===t?`active`:``,a=e.titulo||`Documento sin título`;return`
